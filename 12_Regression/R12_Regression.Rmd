...
As a first step, let's explore our data with a combined scatter and correlation plot using the GGally package:

```{r message=FALSE, warning=FALSE}
# use GGally's ggpairs function to make a scatter and correlation plot
ggpairs(insurance.data, columns = c("pct_insur", "pct_unemp", "pct_mar", "pct_65over", "pct_white", "pct_hisp"))
```

This matrix shows the distribution of each variable on the diagonal, bivariate scatterplots in the lower triangle, and correlations in the upper triangle. From this, we can see a probable negative relationship between percent insured ( _pct\_insur_) and percent unemployed ( _pct\_unemp_) and hispanic ( _pct\_hisp_), while there is a positive relationship between percent insured and the percent married ( _pct\_mar_), over 65 ( _pct\_65over_), and white ( _pct\_white_). 

To see the strength of these relationships, let's specify our multiple regression using these variables:
...
The error model output is quite similar, with the main difference being that the strength of spatial association is now shown by the $\lambda$ autoregressive parameter. In comparison with the spatial lag model output, the spatial error model performs worse, based on their respective AICs and Log Likehood. But using an analysis of variation (ANOVA) model, we can see that in general both seem to perform about equally (one is not significantly different from the other): 
