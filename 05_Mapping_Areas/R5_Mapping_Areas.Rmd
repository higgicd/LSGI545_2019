...
As was shown in the previous practical (see R2 Data Manipulation in R), we can use the plot() function which is built into base R to show the outlines of the polygons contained within the "LSOA" object.
...
We need to choose some colors that we will assign to each of the break points in the data. We will now use another package called "RColorBrewer" which provides a series of color palettes that are suitable for mapping. You can have a look at the color palettes online: http://colorbrewer2.org. Each of these palettes are named; and you can see all the available palettes as follows.
...
We will then choose six colors from the palette "YlOrRd", and print them to the terminal. You will see that the colors are stored as [hex values](http://www.color-hex.com/).
...
```{r}
WARD <- readOGR(dsn = "./data", layer = "england_cmwd_2011Polygon")
```
...
Next we can add a style to the map - this includes the variable to use for coloring ("col="), in this case, the rank of the area by the [Index of Multiple Deprivation](https://www.gov.uk/government/statistics/english-indices-of-deprivation-2015). The style is set as "equal" which relates to how the IMD ranks are broken up into color bins; the border color ("border.col=") and transparency (" border.alpha ="); title and also palette for the choropleth ("palette = ") which uses the colorbrewer palettes. Finally, the "showNA" option can be set as true or false and relates to whether missing values are shown in the legend.

```{r}
m <- tm_shape(Leeds, projection=27700) +
	tm_polygons(col="imd_rank", style="equal",n=5,border.col = "grey50",  border.alpha = .25, title="IMD Quintile", showNA=FALSE,palette="Blues")
#Print plot
m
```
...
tm_shape(LSOA, projection=27700) +
	tm_polygons(col="imd_rank", style="cont",n=5,border.col = "grey50",  border.alpha = .25, showNA=FALSE,palette="Greys",title="IMD Rank") +
# Add roads
tm_shape(roads) +
...
# Plot
m<- tm_shape(Leeds, projection=27700) +
	tm_polygons(col="imd_rank", style="equal",n=5,border.col = "grey50",  border.alpha = .25, title="IMD Quintile", showNA=FALSE,palette="Greys") +
...
To see why a cartogra is useful, let's first plot a map of population for Leeds based on each area's usual residents in 2011 (UR_2011):

```{r}
#PopulationMap
l <- tm_shape(Leeds) +
	tm_polygons(col="UR_2011", style="equal",n=5,border.col = "grey50",  border.alpha = .25, title="Usual Residents in 2011", showNA=FALSE,palette="Blues") +
    	tm_layout(legend.position = c("left", "bottom"), 
			  frame = FALSE)
...
As you can see, most of the population is in a few small areas, while larger areas with lower population dominate the map visually. A cartogram can correct for this by altering the size of the areas based on their population.
...
```{r eval=FALSE}
library("cartogram")
Leeds_Cartogram <- cartogram_cont(Leeds, "UR_2011", itermax = 15, maxSizeError = 1.0001,prepare = "adjust", threshold = 0.05) # Takes a long time to process. May give some mean size errors - this is OK
```

If you don't want to make the cartogram yourself, you can load a pre-made shapefile from the exercise data:

```{r echo=FALSE, warning=FALSE,message=FALSE}
Leeds_Cartogram <- readOGR("./data","Leeds_Cartogram",verbose = FALSE)
```

We can now create the same population map using the cartogram:

```{r}
#CartogramPopulationMap
lc <- tm_shape(Leeds_Cartogram) +
	tm_polygons(col="UR_2011", style="equal",n=5, border.col = "grey50",  border.alpha = .25, title="Usual Residents in 2011", showNA=FALSE,palette="Blues") +
    	tm_layout(legend.position = c("left", "bottom"), 
			  frame = FALSE)
...
Now we can make another choropleth of deprivation, however, now we have corrected the visual representation of deprivation by each area's population:

```{r}
#Map
mc <- tm_shape(Leeds_Cartogram) +
	tm_polygons(col="imd_rank", style="equal",n=5,border.col = "grey50",  border.alpha = .25, title="IMD Quintile", showNA=FALSE,palette="Blues") +
    	tm_layout(legend.position = c("left", "bottom"), 
			  frame = FALSE)
...
This distortion makes cartograms an effective tool for choropleth mapping because the area of the polygon features is based off of the variable you are mapping. In traditional choropleth maps, you might have cases where a large area with a small population visually dominates your map while smaller areas with large populations are not given as much visual emphasis. This is common with maps of census variables, as areas with larger populations tend to have smaller polygons.
