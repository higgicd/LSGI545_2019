...
* Map bike share flows in San Francisco
* Map GPS tracking data in Beijing
* Manipulate LODES data from census blocks into an aggregate geography for Texas
* Format origin-destination data flow so that it is suitable for mapping
...
Now let's get our background map:
...
```{r}
#Get background map for San Francisco
...
SanFran <- get_stamenmap(bbox = c(-122.429069,37.766395,-122.377871,37.811909), zoom = 14, maptype="toner-lite")
...
For this you will need to get a [Google maps API](https://developers.google.com/maps/documentation/directions/) key, and Google now requires you to set up billing. *If you don't have a key, you can skip to Line 271 where some pre-made data can be loaded.*
...
```{r}
ggmap(SanFran) +
  geom_path(aes(x = lon, y = lat), color = "red", size = 1, data = df_polyline, lineend = "round")
...
```{r}
ggmap(SanFran) +
  geom_path(aes(x = lon, y = lat,color = as.factor(ID)), size = 1, data = tmp, lineend = "round")
```

The code below will *geocode ALL the cycling flows for visualization*. You will need your own Google API key to do this. *Don't run unless you have several hours to kill!* The tracks have already been geocoded below - skip to the next code chunk at Line 271.
...
beijing <- get_stamenmap(bbox = c(116.261041,39.943363,116.37456,40.033325), zoom = 13, maptype="toner-lite")
...
```{r message=FALSE, error=FALSE}
# Import spatial data
TX_SP <- readOGR(dsn = "./data/Texas_Tract.geojson", layer = "Texas_Tract")
...
```{r message=FALSE, error=FALSE}
# Get base map
# let's use tmaptools' geocode OSM feature to automatically get the bounding box for stamen maps:
library(tmaptools)
texas <- get_stamenmap(rbind(as.numeric(paste(geocode_OSM("Texas")$bbox))), zoom = 6, maptype="toner-lite")
```
...
Houston <- get_stamenmap(rbind(as.numeric(paste(geocode_OSM("Houston, TX")$bbox))), zoom = 10, maptype="toner-lite")
...
# Create plot
ggmap(Houston,darken = 0.8) + 
      geom_segment(data=OD_Tract[OD_Tract$S000 > 5,],aes(y = h_lat, x = h_lon, yend = w_lat, xend = w_lon, alpha= S000), size=0.4, colour = "white") +
