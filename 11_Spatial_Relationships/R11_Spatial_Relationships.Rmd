...
The stats refer to the key values in the boxplot, the n is the number of observations, conf is a confidence interval for the median, and out are is a list of the outliers in the data. We can use the information provided by `bopxplot.stats()` to add values (labels) to our boxplot, combined with a simple `text()` function after our plot: 
...
In R, Pearson's Correlation Coefficient can be easily computed using the `cor()` function. For a very basic example, we will use Airbnb data for Washington, DC and do some analysis for a snapshot of the offered accommodation. There are various repositories online that can be used to download Airbnb data, such as [insideairbnb.com](http://insideairbnb.com/get-the-data.html) or [tomslee.net](http://tomslee.net/airbnb-data-collection-get-the-data).
...
```{r }
# Fix cleaning_fee
airbnb.dc$cleaning_fee <- substring(airbnb.dc$cleaning_fee, 2) # keep digits after the first character ($ sign)
...
Besides descriptive statistics, the `summary` function also gives information about the amount of missing data in our data; in this case 11 NAs are present in the variable _beds_.
...
#dc.map <- get_stamenmap(location=dc.location, source="stamen", maptype="toner", zoom=13, crop=FALSE)

dc.map <- get_stamenmap(bbox=dc.location, maptype="toner-lite", zoom=13, crop=FALSE)

# Make a map with the airbnb points
airbnb.dc.map <- ggmap(dc.map) + 
                 geom_point(aes(x=longitude, y=latitude, color=price_cat), 
                            data=airbnb.dc, alpha = 0.9, size = 0.9) +
...
In this case we will further explore the relationships between location and magnitude using a _spatial regression model_. However, these models can be quite complex, so before that we will look at simple regression models first in the next exercise.
