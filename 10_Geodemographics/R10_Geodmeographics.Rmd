...
First we will load a series of tables which includes: "Liverpool","Census_2011_Count","Variable_Desc" and "OAC_Input_Lookup". Census_2011_Count_All is a data frame containing census data for the UK, these are detailed in "Variable_Desc". The Liverpool data frame is a list of output areas within Liverpool, UK; however, if you wanted to repeat this analysis for a different city, you could swap this list. Finally, "OAC_Input_Lookup" is a lookup for the variables used to create OAC. Let's load the data with input prepared for Liverpool, UK:
...
You will see that there are no large decreases in the within sum of squares, and a minor moderation of the decrease around 7 or 8 clusters; which also mirrors similar patterns observed within UK OAC. As such, let's choose a 7 cluster solution.

## Building the geodemographic classification
...
```{r eval= FALSE}
# you can run this if you want - it will take a few minutes to run the kmeans clustering. If you don't want to wait, just run the Rdata chunk below
...
# Import OA boundaries (windows)
liverpool_SP <- readOGR(dsn = "./data/Liverpool_OA_2011.geojson", layer = "Liverpool_OA_2011")
...
To assist with spotting trends within the grand index table we can create a plot of shaded cells. Before doing this we will however need to convert the data into a narrow format. The simplest way of going this is with the melt function that is contained within the reshape2 package.
...
From this we can see that Cluster G for example has higher rates of unemployment, high rates of social rented housing, very high rates of households that have only limited ability to speak English, and very high rates of Black, Chinese, Bangladeshi, Pakistani, Indian, and Mixed ethnicities relative to the average.
